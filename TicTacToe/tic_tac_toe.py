...
def check_win(player, pos1: ScratchVar, pos2: ScratchVar, pos3: ScratchVar, pos4: ScratchVar, pos5: ScratchVar, pos6: ScratchVar, pos7: ScratchVar, pos8: ScratchVar, pos9: ScratchVar):
    return If(Or(And(pos1.load() == player, pos2.load() == player, pos3.load() == player),
       ...
       And(pos3.load() == player, pos5.load() == player, pos7.load() == player))).Then(player).Else(Int(0))
    ...
    return Seq(
        ...
        For(i.store(Int(0)), i.load() < Len(player_moves), i.store(i.load() + Int(1))).Do(
            ...
            For(j.store(Int(1)), j.load() < Int(10), j.store(j.load() + Int(1))).Do(
                ...
                .ElseIf(j.load() == Int(2)).Then(pos.set_index(pos2))
                .ElseIf(j.load() == Int(3)).Then(pos.set_index(pos3))
                .ElseIf(j.load() == Int(4)).Then(pos.set_index(pos4))
                .ElseIf(j.load() == Int(5)).Then(pos.set_index(pos5))
                .ElseIf(j.load() == Int(6)).Then(pos.set_index(pos6))
                .ElseIf(j.load() == Int(7)).Then(pos.set_index(pos7))
                .ElseIf(j.load() == Int(8)).Then(pos.set_index(pos8))
                .Else(pos.set_index(pos9)),
                If(Not(pos.load())).Then(
                    pos.store(Int(2)),
                    If(check_win(Int(2), pos1, pos2, pos3, pos4, pos5, pos6, pos7, pos8, pos9)).Then(
                        moved.store(Int(1)),
                        Break()
                    ).Else(
                        pos.store(Int(0))
